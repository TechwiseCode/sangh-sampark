...
<head>
  ...
  <title><?php echo htmlspecialchars(page_title(t('auth.super_title'))); ?></title>
...
<body class="<?php echo trim(subtle_accent_body_class()); ?>">
        ...
        <div class="row w-100 mx-0">
          ...
          <div class="col-lg-4 mx-auto">
            <div class="auth-form-light text-left py-5 px-4 px-sm-5 text-center">
              ...
              <h4><?php echo h('auth.super_welcome'); ?></h4>
              <h6 class="fw-light"><?php echo h('auth.super_subtitle'); ?></h6>
              <div id="login-alert" class="alert alert-danger mt-3" style="display:none;"></div>
              <form class="pt-3" id="login-form" action="<?php echo htmlspecialchars(base_url()); ?>/login" method="post" autocomplete="on">
                <div class="form-group">
                  <input type="text" class="form-control form-control-lg" name="identity" id="identity" placeholder="<?php echo h('auth.identity_placeholder'); ?>" required autocomplete="username" inputmode="email">
                </div>
                <div class="form-group">
                  <div class="password-field">
                    <input type="password" class="form-control form-control-lg password-field__input" name="password" id="password" placeholder="<?php echo h('auth.password_placeholder'); ?>" required autocomplete="current-password">
                    <button type="button" class="password-field__toggle js-toggle-password" data-target="password" aria-label="Show password">
                ...
                <div class="auth-remember-row">
                  <div class="form-check">
                    <input type="checkbox" class="form-check-input" id="remember-me-super" name="remember_me" value="1">
                    <label class="form-check-label" for="remember-me-super"><?php echo h('auth.remember_me'); ?></label>
                  </div>
                </div>
                <div class="mt-3">
                  <button type="submit" class="btn btn-lg btn-success btn-block" id="login-btn"><?php echo h('auth.super_signin_btn'); ?></button>
                </div>
                <div class="my-2 text-center auth-form-links">
                  <a class="auth-link text-black small d-block" href="<?php echo htmlspecialchars(base_url()); ?>/login"><?php echo h('auth.org_link_from_super'); ?></a>
                </div>
              </form>
            </div>
          </div>
  ...
  (function () {
    ...
    var rememberMeEl = document.getElementById('remember-me-super');
    var store = window.SanghSamparkLoginStore ? window.SanghSamparkLoginStore.superadmin : null;
    ...
    function saveLoginDetails(identity, password) {
      if (!rememberEnabled()) {
        if (store) {
          store.clear();
        }
      ...
      if (store) {
        store.write({ remember: true, identity: identity, password: password });
      }
      return rememberApi.saveBrowserCredential
        ? rememberApi.saveBrowserCredential({ identity: identity, password: password })
        : Promise.resolve();
    }
    ...
    function submitLogin() {
      ...
      fetch(base + '/login', {
        ...
        .then(function (res) {
          ...
          if (res.body.ok) {
            if (res.body.user && res.body.user.role === 'superadmin') {
              try {
                saveLoginDetails(identity, password);
              } catch (e) {}
              window.location.href = base + '/superadmin';
            } else {
              alertEl.textContent = 'This sign-in page is only for platform superadmin accounts.';
            ...
            if (res.status === 404) {
              alertEl.textContent = <?php echo json_encode(t('auth.js_endpoint_404_super')); ?>.replace('{url}', base + '/login');
            } else {
